...
const logger = require('log4js').getLogger('commands.blockchain.services.tmTxMetadata');

/**
 *
 * @param {*} model the request object
 * @param {*} tmTxMetadata the name of the model
 * @return {*} model the record of the model
 */
function create(model, tmTxMetadata) {
  logger.trace('execute commands.blockchain.services.tmTxMetadata');
  logger.debug('model :', model);
  model.create(tmTxMetadata);
  return tmTxMetadata;
}
